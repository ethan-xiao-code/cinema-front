  ...
  <div id="home">
    ...
    <el-form :inline="true" class="filter-bar" label-width="0">
      <div class="filter-bar-right">
        ...
        <!-- 按钮 -->
        <el-form-item>
          <el-button type="primary" @click="qeuryChartData">查询</el-button>
...
const getFilmBoxOffice = async () => {
  const res = await getFilmBoxOfficeApi({ ...params.value })
  filmList.value = res.map(item => {
    const {filmName} = item
...
const getMonthTicket = async () => {
  const res = await getMonthTicketApi({ ...params.value })
  monthTicketList.value = res || []

}

const qeuryChartData = async () => {
  if (dateRange.value.length === 2) {
    params.value.startTime = dayjs(dateRange.value[0]).startOf('day').format("YYYY-MM-DD HH:mm:ss")
    params.value.endTime = dayjs(dateRange.value[1]).endOf('day').format("YYYY-MM-DD HH:mm:ss")
  }
  await Promise.all([getFilmBoxOffice(), getMonthTicket()])
}
